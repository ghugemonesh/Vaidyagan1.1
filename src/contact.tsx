...
import { Mail, MapPin, Send, Instagram, Check } from "lucide-react";
import { getConsoleSettings } from "./console/db";

export function Contact() {
  const { toast } = useApp();
  const contactEmail = (() => { try { return getConsoleSettings().contactEmail || "vaidyagan@gmail.com"; } catch { return "vaidyagan@gmail.com"; } })();
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      window.location.href = `mailto:${contactEmail}?subject=${encodeURIComponent(form.subject)}&body=${body}`;
  ...
  const input = "w-full rounded-xl border border-forest-700 bg-forest-900/80 px-4 py-3 text-[14.5px] text-sand-100 placeholder:text-sand-200/35 transition-all focus:border-gold-400 focus:outline-none";
        ...
        <Reveal>
          {sent ? (
            <div className="flex h-full flex-col items-center justify-center rounded-2xl border border-kapha-500/40 bg-kapha-500/6 p-12 text-center">
              ...
              <p className="mt-5 font-display text-2xl font-semibold">Message received</p>
            ...
            <form onSubmit={submit} className="rounded-2xl border border-forest-800 bg-forest-900/60 p-7 sm:p-9">
              ...
              <button type="submit" disabled={sending} className="gold-sheen mt-6 flex w-full items-center justify-center gap-2 rounded-full bg-gold-400 py-3.5 font-mono text-[11px] font-semibold uppercase tracking-[0.18em] text-forest-950 transition-all hover:bg-gold-300 disabled:opacity-60">
                {sending ? <span className="animate-spin-fast inline-block h-4 w-4 rounded-full border-2 border-forest-950 border-t-transparent" /> : <Send size={15} />} {sending ? "Sending…" : "Send message"}
              </button>
              <p className="mt-3 text-center font-mono text-[9px] uppercase tracking-[0.14em] text-sand-200/35">Delivered to {contactEmail}</p>
            </form>
          )}
        </Reveal>
          ...
          <Reveal delay={100}>
            <div className="rounded-2xl border border-forest-800 bg-forest-900/60 p-6">
              ...
              <div className="mt-4 space-y-3.5">
                <a href={`mailto:${contactEmail}`} className="flex items-center gap-3 text-[13.5px] text-sand-200/75 transition-colors hover:text-gold-300"><Mail size={16} className="text-gold-400/70" /> {contactEmail}</a>
