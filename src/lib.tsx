...
export interface StudioUser extends StudioPerms {
  ...
  username: string; password: string; specialty?: string; hue: string; active: boolean; createdAt: string;
}
...
const SEED_USERS: StudioUser[] = [
  ...
  { ...DEFAULT_PERMS, id: "shruti", name: "Dr. Shruti Choudhary", role: "doctor", username: "shruti", password: "shruti123", specialty: "Dravyaguna · Clinical herbology", hue: "#82b39e", active: true, createdAt: "2025-03-12" },
  { ...DEFAULT_PERMS, id: "bhagyesh", name: "Dr. Bhagyesh Karale", role: "doctor", username: "bhagyesh", password: "bhagyesh123", specialty: "Panchakarma · Detox protocols", hue: "#e07f49", active: true, createdAt: "2025-04-02" },
  { ...DEFAULT_PERMS, id: "shivani", name: "Dr. Shivani Kadam", role: "doctor", username: "shivani", password: "shivani123", specialty: "Stri Roga · Women's health", hue: "#93b1cf", active: true, createdAt: "2025-05-18" },
];
...
export const auth = {
  ...
  addMember(input: Omit<StudioUser, "id" | "active" | "createdAt" | "hue" | keyof StudioPerms> & Partial<StudioPerms> & { hue?: string }): { ok: boolean; error?: string; user?: StudioUser } {
    const users = loadUsers();
    if (users.some((u) => u.username.toLowerCase() === input.username.trim().toLowerCase())) return { ok: false, error: "That username is already taken." };
    const user: StudioUser = {
      ...DEFAULT_PERMS, ...input, username: input.username.trim(),
      id: `u-${Date.now()}`, active: true, hue: input.hue || "#82b39e", createdAt: new Date().toISOString().slice(0, 10),
    };
  ...
  setRole(id: string, role: "superadmin" | "doctor"): { ok: boolean; error?: string } {
    ...
    persistUsers(users.map((u) => (u.id === id ? { ...u, role, ...(role === "superadmin" ? { storeAccess: true, consoleAccess: true } : {}) } : u)));
    return { ok: true };
  },
...
export interface ActivityEntry { id: string; actor: string; kind: string; action: string; target?: string; at: string }
...
const DELETED_POSTS_KEY = "vaidyagan_deleted_posts_v1";
...
function loadPosts(): Article[] {
  try {
    ...
    if (raw) { const parsed = JSON.parse(raw) as Article[]; if (Array.isArray(parsed)) return parsed; }
  } catch { /* fresh */ }
  return [];
}
...
function loadDeletedPostIds(): string[] {
  try {
    ...
    if (raw) { const parsed = JSON.parse(raw) as string[]; if (Array.isArray(parsed)) return parsed; }
  } catch { /* fresh */ }
  return [];
}
...
function loadHerbs(): Herb[] {
  try {
    ...
    if (raw) { const parsed = JSON.parse(raw) as Herb[]; if (Array.isArray(parsed) && parsed.length > 0) return parsed; }
  } catch { /* seeds */ }
  return HERBS;
}
function loadProducts(): Product[] {
  try {
    ...
    if (raw) { const parsed = JSON.parse(raw) as Product[]; if (Array.isArray(parsed) && parsed.length > 0) return parsed; }
  } catch { /* seeds */ }
  return PRODUCTS;
}
function loadOrders(): Order[] {
  try {
    ...
    if (raw) { const parsed = JSON.parse(raw) as Order[]; if (Array.isArray(parsed)) return parsed; }
  } catch { /* seeds */ }
  return SEED_ORDERS;
}
function loadActivity(): ActivityEntry[] {
  try {
    ...
    if (raw) { const parsed = JSON.parse(raw) as ActivityEntry[]; if (Array.isArray(parsed)) return parsed; }
  } catch { /* fresh */ }
  return [];
}
function persistActivity(list: ActivityEntry[]) { try { localStorage.setItem(ACTIVITY_KEY, JSON.stringify(list)); } catch { /* ignore */ } }
function loadFlag(key: string, def: boolean): boolean {
  try { const raw = localStorage.getItem(key); if (raw !== null) return raw === "1"; } catch { /* default */ }
  return def;
}
...
function pushNotif(n: { title: string; body: string; icon: string }) {
  try {
    ...
    const list: { id: string; title: string; body: string; icon: string; read: boolean; at: string }[] = raw ? JSON.parse(raw) : [];
...
export interface Address { id: string; label: "Home" | "Work" | "Other"; name: string; phone: string; line1: string; city: string; state: string; pin: string; isDefault: boolean }
...
const SEED_CUSTOMERS: Customer[] = [
  { id: "c-1", name: "Rohini Deshpande", phone: "+91 98220 11223", email: "rohini@example.com", provider: "otp", addresses: [{ id: "a-1", label: "Home", name: "Rohini Deshpande", phone: "+91 98220 11223", line1: "14, Sadashiv Peth", city: "Pune", state: "Maharashtra", pin: "411030", isDefault: true }], createdAt: new Date(Date.now() - 40 * 86400e3).toISOString() },
  { id: "c-2", name: "Kunal Mehta", phone: "+91 99870 44556", email: "kunal@example.com", provider: "google", addresses: [{ id: "a-2", label: "Home", name: "Kunal Mehta", phone: "+91 99870 44556", line1: "B-702, Green Acres, Baner", city: "Pune", state: "Maharashtra", pin: "411045", isDefault: true }], createdAt: new Date(Date.now() - 25 * 86400e3).toISOString() },
  { id: "c-3", name: "Meera Iyer", phone: "+91 90040 77889", email: "meera@example.com", provider: "email", password: "meera123", addresses: [{ id: "a-3", label: "Home", name: "Meera Iyer", phone: "+91 90040 77889", line1: "22, Lakeview Road", city: "Mumbai", state: "Maharashtra", pin: "400071", isDefault: true }], createdAt: new Date(Date.now() - 12 * 86400e3).toISOString() },
  { id: "c-4", name: "Arvind Kulkarni", phone: "+91 97650 33445", email: "arvind@example.com", provider: "otp", addresses: [{ id: "a-4", label: "Home", name: "Arvind Kulkarni", phone: "+91 97650 33445", line1: "5, Shivaji Nagar", city: "Nashik", state: "Maharashtra", pin: "422001", isDefault: true }], createdAt: new Date(Date.now() - 5 * 86400e3).toISOString() },
];

function loadCustomers(): Customer[] {
  try {
    const raw = localStorage.getItem(CUSTOMERS_KEY);
    if (raw) { const parsed = JSON.parse(raw) as Customer[]; if (Array.isArray(parsed) && parsed.length > 0) return parsed; }
  } catch { /* fresh */ }
  return SEED_CUSTOMERS;
}
function persistCustomers(list: Customer[]) { try { localStorage.setItem(CUSTOMERS_KEY, JSON.stringify(list)); } catch { /* ignore */ } }
...
interface AppCtx {
  ...
  updateOrderStatus: (id: string, s: OrderStatus) => void; cancelOrder: (id: string) => void; cancelAndRestock: (id: string) => void;
  storeEnabled: boolean; setStoreEnabled: (b: boolean) => void;
  profileTabEnabled: boolean; setProfileTabEnabled: (b: boolean) => void;
  cart: CartLine[]; addToCart: (id: string, qty?: number) => void; changeQty: (id: string, d: number) => void; removeLine: (id: string) => void;
  ...
  activity: ActivityEntry[]; logActivity: (kind: ActivityKind, action: string, target?: string) => void;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [storeEnabled, setStoreEnabledState] = useState<boolean>(() => loadFlag(STORE_FLAG_KEY, true));
  const [profileTabEnabled, setProfileTabState] = useState<boolean>(() => loadFlag(PROFILE_TAB_KEY, true));
  ...
  const navigate = useCallback((v: View) => {
    ...
    try {
      const KEY = "vaidyagan_pageviews_v1";
      const raw = localStorage.getItem(KEY);
      const list: { id: string; page: string; at: string }[] = raw ? JSON.parse(raw) : [];
      list.unshift({ id: `pv-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`, page: v.name, at: new Date().toISOString() });
  ...
  const logActivity = useCallback((kind: ActivityKind, action: string, target?: string) => {
    setActivity((list) => {
      const entry: ActivityEntry = {
        ...
        actor: auth.session()?.name ?? "System", kind, action, target, at: new Date().toISOString(),
      };
  ...
  /* articles: seed classics (minus deleted) overlaid with desk posts */
  const allArticles = useMemo(() => {
    const desk = new Map(userPosts.map((p) => [p.id, p]));
    const out: Article[] = [...userPosts];
    for (const a of ARTICLES) {
      if (deletedPostIds.includes(a.id)) continue;
      if (!desk.has(a.id)) out.push(a);
    }
    return out;
  ...
  const deleteUserPost = useCallback((id: string) => {
    setUserPosts((list) => { const next = list.filter((x) => x.id !== id); persistPosts(next); return next; });
  }, []);

  /** Delete any article — classics are recorded as deleted so they vanish site-wide. */
  const deleteArticle = useCallback((id: string) => {
    const src = ARTICLES.find((a) => a.id === id);
    if (src) {
      setDeletedPostIds((ids) => { const next = ids.includes(id) ? ids : [...ids, id]; persistDeletedPostIds(next); return next; });
      setUserPosts((list) => { const next = list.filter((x) => x.id !== id); persistPosts(next); return next; });
      return;
    }
    deleteUserPost(id);
  ...
  const placeOrder = useCallback((cust: OrderCustomer, paymentMethod?: string, extra?: { discountCode?: string; discountAmount?: number; shippingFee?: number }): Order => {
    ...
    setProducts((list) => {
      const next = list.map((p) => {
        ...
        if (stock > 0 && stock < 5) {
          try { pushNotif({ title: `Low stock — ${p.name}`, body: `Only ${stock} unit${stock === 1 ? "" : "s"} left after order ${order.id}.`, icon: "stock" }); } catch { /* ignore */ }
        }
        return { ...p, stock };
      });
      try { localStorage.setItem(PRODUCTS_KEY, JSON.stringify(next)); } catch { /* ignore */ }
  ...
  const updateOrderStatus = useCallback((id: string, s: OrderStatus) => {
    setOrders((o) => { const next = o.map((x) => (x.id === id ? { ...x, status: s } : x)); try { localStorage.setItem(ORDERS_KEY, JSON.stringify(next)); } catch { /* ignore */ } return next; });
  }, []);
  ...
  const loginOtp = useCallback((phone: string, name?: string): Customer => {
    ...
    const existing = list.find((c) => c.phone.replace(/\D/g, "") === phone.replace(/\D/g, ""));
  ...
  const saveAddress = useCallback((a: Address) => {
    ...
    setCustomer((c) => {
      if (!c) return c;
      let addresses = c.addresses.some((x) => x.id === a.id) ? c.addresses.map((x) => (x.id === a.id ? a : x)) : [...c.addresses, a];
  ...
  const myOrders = useMemo(() => (customer ? orders.filter((o) => o.customerId === customer.id) : []), [orders, customer]);

  const value: AppCtx = {
    view, navigate, articles, allArticles, userPosts, saveDraft, publishArticle, deleteUserPost, deleteArticle,
    herbs, saveHerb, deleteHerb, resetHerbs, products, saveProduct, deleteProduct,
    ...
    cart, addToCart, changeQty, removeLine, cartOpen, setCartOpen,
    ...
    activity, logActivity, toasts, toast,
  };
...
export function Reveal({ children, className = "", delay = 0, as: Tag = "div" }: { children: React.ReactNode; className?: string; delay?: number; as?: any }) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const io = new IntersectionObserver((entries) => entries.forEach((e) => { if (e.isIntersecting) { setSeen(true); io.disconnect(); } }), { threshold: 0.12 });
    io.observe(el);
...
export function SectionHead({ eyebrow, title, sub }: { eyebrow: string; title: React.ReactNode; sub?: string }) {
  return (
    <Reveal className="max-w-2xl">
      ...
      <h2 className="mt-4 font-display text-3xl font-semibold leading-tight text-sand-100 sm:text-[2.6rem] sm:leading-[1.08]">{title}</h2>
...
export function Chip({ children, active, onClick, color }: { children: React.ReactNode; active?: boolean; onClick?: () => void; color?: string }) {
  return (
    <button onClick={onClick}
      className={`rounded-full border px-4 py-1.5 font-mono text-[10px] uppercase tracking-[0.14em] transition-all duration-300 ${active ? "text-forest-950" : "text-sand-200/60 hover:text-sand-100"}`}
...
export function Monogram({ author, size = 40 }: { author: { initials: string; hue: string }; size?: number }) {
  return (
    <span className="grid shrink-0 place-items-center rounded-full font-display font-semibold"
...
export function GoldButton({ children, onClick, className = "" }: { children: React.ReactNode; onClick?: () => void; className?: string }) {
  return (
    <button onClick={onClick}
      className={`gold-sheen group inline-flex items-center gap-2 rounded-full bg-gold-400 px-6 py-3 font-mono text-[11px] font-semibold uppercase tracking-[0.18em] text-forest-950 transition-all duration-300 hover:bg-gold-300 hover:shadow-[0_0_28px_rgba(214,180,95,0.35)] active:scale-95 ${className}`}>
      {children}
    </button>
  );
}
...
export function SmartImg({ src, alt, className = "", style }: { src: string; alt: string; className?: string; style?: React.CSSProperties }) {
  ...
  if (err || !src) {
    return (
      <div className={`grid place-items-center bg-forest-850 ${className}`} style={style} aria-hidden>
        <span className="font-display text-3xl italic text-gold-500/30">वै</span>
      </div>
    );
  }
...
export function Stars({ rating, size = 13, onRate }: { rating: number; size?: number; onRate?: (n: number) => void }) {
  return (
    <span className="inline-flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <button key={n} disabled={!onRate} onClick={() => onRate?.(n)} aria-label={`${n} star${n > 1 ? "s" : ""}`} className={onRate ? "transition-transform hover:scale-125" : "cursor-default"}>
