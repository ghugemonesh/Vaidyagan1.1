...
import {
  ARTICLES, AUTHORS, HERBS, PRODUCTS, SEED_ORDERS,
  type Article, type Author, type Herb, type Order, type OrderCustomer, type OrderStatus, type Product,
} from "./data";
...
export interface StudioPerms {
  canPublishDirect: boolean; canEditPublished: boolean; canDeletePublished: boolean;
  storeAccess: boolean; herbAccess: boolean; consoleAccess: boolean;
  consoleRole: "superadmin" | "editor" | "viewer";
}
...
export const DEFAULT_PERMS: StudioPerms = {
  canPublishDirect: true, canEditPublished: true, canDeletePublished: false,
...
const SUPERADMIN: StudioUser = {
  ...DEFAULT_PERMS, id: "root", name: "Monesh", role: "superadmin", username: "monesh", password: "admin91466",
  specialty: "Founder · Vaidyagan", hue: "#d6b45f", active: true, createdAt: "2024-01-01",
  storeAccess: true, consoleAccess: true, consoleRole: "superadmin", canDeletePublished: true,
};
...
function loadUsers(): StudioUser[] {
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) return parsed.map((u) => ({ ...DEFAULT_PERMS, ...u }));
    }
  } catch { /* seeds */ }
...
export const auth = {
  ...
  addMember(data: Omit<StudioUser, "id" | "active" | "createdAt" | "hue" | keyof StudioPerms> & Partial<StudioPerms> & { hue?: string }): { ok: boolean; error?: string; user?: StudioUser } {
    const users = loadUsers();
    if (users.some((u) => u.username.toLowerCase() === data.username.trim().toLowerCase())) return { ok: false, error: "That username is already taken." };
    const user: StudioUser = {
      ...DEFAULT_PERMS, ...data, username: data.username.trim(),
      id: `u-${Date.now()}`, active: true, hue: data.hue || "#82b39e", createdAt: new Date().toISOString().slice(0, 10),
    };
  ...
  setRole(id: string, role: "superadmin" | "doctor"): { ok: boolean; error?: string } {
    ...
    persistUsers(users.map((u) => (u.id === id ? { ...u, role, ...(role === "superadmin" ? { storeAccess: true, consoleAccess: true, consoleRole: "superadmin" as const } : {}) } : u)));
    return { ok: true };
  },
  ...
  removeMember(id: string): boolean {
    ...
    persistUsers(loadUsers().filter((u) => u.id !== id));
    return true;
  },
};
...
export type ActivityKind = "publish" | "approve" | "reject" | "member" | "store" | "edit" | "submit" | "system";
...
function loadStoreFlag(): boolean {
  try { const raw = localStorage.getItem(STORE_FLAG_KEY); if (raw !== null) return raw === "1"; } catch { /* default */ }
  return true;
}
function loadProfileTabFlag(): boolean {
  try { const raw = localStorage.getItem(PROFILE_TAB_KEY); if (raw !== null) return raw === "1"; } catch { /* default */ }
  return true;
}

/* -------------------------- desk doctors (roster) --------------------------- */
function loadDeskDoctors(): Author[] {
  try {
    const raw = localStorage.getItem(DESK_DOCTORS_KEY);
    if (raw) {
...
function persistDeskDoctors(list: Author[]) {
  try { localStorage.setItem(DESK_DOCTORS_KEY, JSON.stringify(list)); } catch { /* ignore */ }
}
...
interface AppCtx {
  ...
  deskDoctors: Author[]; addDeskDoctor: (a: Author) => void; removeDeskDoctor: (id: string) => void;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [deskDoctors, setDeskDoctors] = useState<Author[]>(loadDeskDoctors);
  ...
  /* desk doctors roster */
  const addDeskDoctor = useCallback((a: Author) => {
    setDeskDoctors((list) => {
      const next = list.some((x) => x.id === a.id) ? list : [...list, a];
      persistDeskDoctors(next);
      return next;
    });
  }, []);
  const removeDeskDoctor = useCallback((id: string) => {
    setDeskDoctors((list) => {
      const next = list.filter((x) => x.id !== id);
      persistDeskDoctors(next);
      return next;
  ...
  const loginOtp = useCallback((phone: string, name?: string): Customer => {
    ...
    const existing = list.find((c) => c.phone === phone && !c.suspended);
    ...
    persistCustomers([...list, fresh]);
    startSession(fresh);
    return fresh;
  }, [startSession]);
  const loginGoogle = useCallback((email: string, name: string): Customer => {
    const list = loadCustomers();
    const existing = list.find((c) => c.email.toLowerCase() === email.toLowerCase() && !c.suspended);
    if (existing) { startSession(existing); return existing; }
  ...
  const loginEmail = useCallback((email: string, password: string): Customer | null => {
    const c = loadCustomers().find((x) => x.email.toLowerCase() === email.toLowerCase() && x.password === password && !x.suspended);
    if (!c) return null;
    startSession(c);
    return c;
  }, [startSession]);
  ...
  const updateCustomer = useCallback((patch: Partial<Pick<Customer, "name" | "phone" | "email">>) => {
    setCustomer((c) => {
      if (!c) return c;
      const next = { ...c, ...patch };
      persistCustomers(loadCustomers().map((x) => (x.id === c.id ? next : x)));
      return next;
    });
  }, []);
  ...
  const value: AppCtx = {
    ...
    orders, placeOrder, updateOrderStatus, cancelAndRestock,
    storeEnabled, setStoreEnabled, profileTabEnabled, setProfileTabEnabled,
    deskDoctors, addDeskDoctor, removeDeskDoctor,
    ...
    updateCustomer, saveAddress, deleteAddress, myOrders,
...
export function SectionDivider({ label }: { label?: string }) {
  return (
    <Reveal className="mx-auto flex w-full max-w-5xl items-center gap-5 px-5">
      <span aria-hidden className="gold-rule flex-1" />
      <span className="flex items-center gap-3">
        <span aria-hidden className="h-1.5 w-1.5 rotate-45 bg-gold-500/70" />
        {label && <span className="font-display text-sm italic tracking-wide text-gold-400/80">{label}</span>}
        <span aria-hidden className="h-1.5 w-1.5 rotate-45 bg-gold-500/70" />
      </span>
      <span aria-hidden className="gold-rule flex-1" />
    </Reveal>
...
export function CountUp({ to, duration = 1400, suffix = "" }: { to: number; duration?: number; suffix?: string }) {
  ...
  return <span ref={ref} className="display-num">{val.toLocaleString("en-IN")}{suffix}</span>;
}
...
export function Ticker({ items }: { items: { en: string; sa: string }[] }) {
  ...
  return (
    <div className="ticker-hover relative overflow-hidden border-y border-forest-800 bg-forest-900/60 py-3">
...
export function DoshaDots({ doshas }: { doshas: import("./data").Dosha[] }) {
  const map: Record<string, string> = { vata: "#93b1cf", pitta: "#e07f49", kapha: "#82b39e" };
  return (
...
export function Stars({ rating, size = 13, onRate }: { rating: number; size?: number; onRate?: (n: number) => void }) {
  return (
    <span className="inline-flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <button key={n} disabled={!onRate} onClick={() => onRate?.(n)} aria-label={`${n} star${n > 1 ? "s" : ""}`}
