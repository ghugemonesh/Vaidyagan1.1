...
import { Wind, Flame, Droplets, ArrowRight, Clock, Eye, Search, Sparkles, BookOpen, Leaf, Send } from "lucide-react";
import { useApp, Reveal, ScrambleText, SectionHead, SectionDivider, CountUp, Chip, Ticker, DoshaDots, Monogram, SmartImg, Stars } from "./lib";
import { CATEGORIES, KIND_META, buildSymptomIndex, authorFor, categoryName, formatDate, readingTime, kindOf, type Article, type Dosha } from "./data";
...
export function ArticleCard({ article, delay = 0, big = false }: { article: Article; delay?: number; big?: boolean }) {
  ...
  return (
    <Reveal delay={delay} as="article" className={`group cursor-pointer ${big ? "lg:col-span-2 lg:grid lg:grid-cols-[1.15fr_1fr]" : ""}`}>
      <div onClick={go} className="card-lift flex h-full flex-col overflow-hidden rounded-xl border border-forest-800 bg-forest-900 hover:border-gold-500/50">
        <div className={`relative overflow-hidden ${big ? "aspect-[16/10] lg:aspect-auto lg:h-full" : "aspect-[16/10]"}`}>
          ...
          <div className="absolute left-4 top-4 flex flex-wrap items-center gap-2">
            <span className="rounded-full border bg-forest-950/70 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.16em] backdrop-blur" style={{ borderColor: KIND_META[kind].color, color: KIND_META[kind].color }}>{KIND_META[kind].short}</span>
            <span className="rounded-full border border-gold-500/50 bg-forest-950/70 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.16em] text-gold-300 backdrop-blur">{catName}</span>
            {article.featured && <span className="rounded-full bg-gold-400 px-3 py-1 font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-forest-950">Featured</span>}
          </div>
        ...
        <div className={`flex flex-1 flex-col p-6 ${big ? "lg:p-8" : ""}`}>
          <h3 className={`font-display font-semibold leading-snug text-sand-100 transition-colors duration-300 group-hover:text-gold-300 ${big ? "text-2xl lg:text-[2rem] lg:leading-[1.15]" : "text-xl"}`}>{article.title}</h3>
          ...
          <div className="mt-auto flex items-center justify-between pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[13px] font-semibold text-sand-100">{author.name}</p>
...
const DOSHA_ICON: Record<Dosha, any> = {
  vata: Wind, pitta: Flame, kapha: Droplets,
};

export const DOSHA_META_LOCAL: Record<Dosha, { name: string; sa: string; elements: string; color: string }> = {
...
function DoshaWheel({ onPick, onCore }: { onPick: (d: Dosha) => void; onCore: () => void }) {
  ...
  return (
    <div ref={wrapRef} className="relative mx-auto aspect-square w-full max-w-[560px]" style={{ perspective: "1150px" }}>
      <div aria-hidden className="absolute left-1/2 top-1/2 h-[125%] w-[125%] -translate-x-1/2 -translate-y-1/2 rounded-full" style={{ background: "radial-gradient(circle, rgba(214,180,95,0.10) 0%, rgba(214,180,95,0.03) 42%, transparent 68%)" }} />
      {DUST.map((p, i) => (
        <span key={i} aria-hidden className="animate-floaty absolute rounded-full bg-gold-400/30" style={{ left: `${p.l}%`, top: `${p.t}%`, width: p.s, height: p.s, animationDelay: `${p.d}s`, animationDuration: `${6.5 + p.d}s` }} />
      ))}

      <div ref={tiltRef} className="absolute inset-0" style={{ transformStyle: "preserve-3d" }}>
        {DOSHA_RING.map((r, i) => {
          const m = DOSHA_META_LOCAL[r.dosha];
          ...
          return (
            <div key={r.dosha} ref={(el) => { ringRefs.current[i] = el; }}
              ...
              <div className="pointer-events-auto absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2" style={{ transformStyle: "preserve-3d", zIndex: 30 }}>
                <div ref={(el) => { nodeRefs.current[i] = el; }} className={`transition-all duration-500 ${dimmed ? "opacity-40 saturate-50" : "opacity-100"}`}>
                  <button onClick={() => onPick(r.dosha)} onMouseEnter={() => setHovered(r.dosha)} onMouseLeave={() => setHovered(null)}
                    aria-label={`Explore ${m.name} — ${m.elements}`} className="group relative grid cursor-pointer place-items-center">
                    <span aria-hidden className="absolute -inset-7 rounded-full blur-2xl transition-opacity duration-500" style={{ background: `radial-gradient(circle, ${m.color}66 0%, transparent 70%)`, opacity: isHover ? 1 : 0 }} />
        ...
        <div className="pointer-events-auto absolute left-1/2 top-1/2 z-[40]" style={{ transform: "translate(-50%,-50%) translateZ(150px)" }}>
          <button onClick={onCore} aria-label="Take the prakriti assessment" className="group relative grid cursor-pointer place-items-center">
    ...
    <div className="relative">
      <div className="flex items-center gap-3 rounded-xl border border-forest-700 bg-forest-900/80 px-5 py-4 transition-all duration-300 focus-within:border-gold-400 focus-within:shadow-[0_0_30px_rgba(214,180,95,0.15)]">
        ...
        <input value={q} onChange={(e) => { setQ(e.target.value); setOpen(true); }} onFocus={() => setOpen(true)}
          ...
          placeholder="Search a symptom — sleep, acidity, memory, stress…" aria-label="Search symptoms"
          className="w-full bg-transparent text-[15px] text-sand-100 placeholder:text-sand-200/35 focus:outline-none" />
        {matches.length > 0 && <span className="hidden shrink-0 font-mono text-[10px] uppercase tracking-[0.18em] text-gold-400/80 sm:block">{matches.length} match{matches.length === 1 ? "" : "es"}</span>}
      </div>
      {open && q.trim() && (
        <div className="absolute inset-x-0 top-full z-30 mt-2 overflow-hidden rounded-xl border border-forest-700 bg-forest-900 shadow-[0_24px_60px_rgba(0,0,0,0.55)]">
          {matches.length === 0 ? (
            ...
            matches.map((m) => (
              <button key={`${m.target.kind}-${m.target.id}-${m.term}`} onMouseDown={(e) => { e.preventDefault(); window.clearTimeout(blurTimer.current); pick(m); }}
                ...
                <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-full ${m.target.kind === "article" ? "bg-gold-400/12 text-gold-400" : "bg-moss-500/15 text-moss-300"}`}>
                  {m.target.kind === "article" ? <BookOpen size={16} /> : <Leaf size={16} />}
                </span>
...
export function Home() {
  const { articles, navigate, deskDoctors } = useApp();
    ...
    <div>
      ...
      <section className="relative overflow-hidden pt-32 sm:pt-36 lg:pt-40">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-14 px-5 pb-20 lg:grid-cols-[1.05fr_1fr] lg:items-center lg:gap-8 lg:px-8 lg:pb-28">
          <div>
            ...
            <Reveal delay={420}>
              <p className="mt-6 max-w-xl text-[15.5px] leading-relaxed text-sand-200/65">
                Classical protocols and herb monographs written from the OPD by registered BAMS vaidyas —
            ...
            <Reveal delay={540} className="mt-8 flex flex-wrap items-center gap-3.5">
              <button onClick={() => navigate({ name: "journal" })} className="gold-sheen group inline-flex items-center gap-2 rounded-full bg-gold-400 px-7 py-3.5 font-mono text-[11px] font-semibold uppercase tracking-[0.18em] text-forest-950 transition-all duration-300 hover:bg-gold-300 hover:shadow-[0_0_28px_rgba(214,180,95,0.35)] active:scale-95">
                Read the journal <ArrowRight size={15} className="transition-transform group-hover:translate-x-1" />
              </button>
              <button onClick={() => navigate({ name: "quiz" })} className="inline-flex items-center gap-2 rounded-full border border-forest-700 px-7 py-3.5 font-mono text-[11px] uppercase tracking-[0.18em] text-sand-200/75 transition-all duration-300 hover:border-gold-400 hover:text-gold-300">
                Take the dosha quiz
              </button>
            </Reveal>
            <Reveal delay={660} className="mt-10 grid max-w-md grid-cols-3 gap-4">
              {([[8, "", "years publishing"], [deskDoctors.length, "", "BAMS vaidyas"], [42, "k", "instagram family"]] as [number, string, string][]).map(([n, sfx, l]) => (
                <div key={l} className="border-l border-gold-500/30 pl-4">
                  <p className="font-display text-2xl font-semibold text-gold-300"><CountUp to={n} suffix={sfx} /></p>
          ...
          <Reveal delay={250} className="relative">
            <DoshaWheel onPick={(d) => { setPicked(d); navigate({ name: "quiz" }); }} onCore={() => navigate({ name: "quiz" })} />
            <AnimatePresence>
              {picked && (
                <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  className="mt-2 text-center font-mono text-[9.5px] uppercase tracking-[0.24em]" style={{ color: DOSHA_META_LOCAL[picked].color }}>
                  {DOSHA_META_LOCAL[picked].name} — {DOSHA_META_LOCAL[picked].elements} · opening the quiz
                </motion.p>
              )}
            </AnimatePresence>
          </Reveal>
      ...
      <section className="relative mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
        <div className="flex flex-wrap items-end justify-between gap-6">
          ...
          <Reveal delay={150}>
            <button onClick={() => navigate({ name: "journal" })} className="gold-sheen group hidden items-center gap-2 rounded-full bg-gold-400 px-6 py-3 font-mono text-[11px] font-semibold uppercase tracking-[0.18em] text-forest-950 transition-all hover:bg-gold-300 sm:inline-flex">
              All essays <ArrowRight size={15} className="transition-transform group-hover:translate-x-1" />
            </button>
          </Reveal>
        </div>
        <Reveal delay={120} className="mt-9 flex flex-wrap gap-2">
          {CATEGORIES.map((c) => (
            <Chip key={c.slug} active={cat === c.slug} onClick={() => setCat(cat === c.slug ? null : c.slug)}>{c.sanskrit} {c.name}</Chip>
          ))}
          <span className="mx-1 hidden w-px bg-forest-700 sm:block" />
          {(["vata", "pitta", "kapha"] as Dosha[]).map((d) => (
            <Chip key={d} active={dosha === d} onClick={() => setDosha(dosha === d ? null : d)} color={DOSHA_META_LOCAL[d].color}>{d}</Chip>
          ))}
        </Reveal>
      ...
      <div className="py-4"><SectionDivider label="चिकित्सक" /></div>

      {/* doctor's corner — hidden if every vaidya switched their page off */}
      {deskDoctors.some((a) => isDoctorListed(a.id)) && (
        <section className="relative border-y border-forest-800 bg-forest-900/60">
          ...
          <div className="relative mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-24">
            <SectionHead eyebrow="Doctor's corner" title={<>The vaidyas behind <em className="text-gold-300">every word</em>.</>}
              sub="No ghostwriters, no aggregators. Registered BAMS practitioners, decades of OPD between them, accountable by name." />
            <div className="no-scrollbar -mx-5 mt-10 flex snap-x gap-5 overflow-x-auto px-5 pb-2 lg:mx-0 lg:px-0">
              {deskDoctors.filter((a) => isDoctorListed(a.id)).map((a, i) => (
                <Reveal key={a.id} delay={i * 90} className="card-lift group w-[300px] shrink-0 snap-start rounded-xl border border-forest-800 bg-forest-950/70 p-6 hover:border-gold-500/50 sm:w-[340px]">
          ...
          <Reveal delay={100}>
            ...
            <div className="mt-8 rounded-xl border border-forest-800 bg-forest-900/70 p-6">
              ...
              <p className="mt-3 text-sm leading-relaxed text-sand-200/65">
                ...
                Sanskrit nidana terms mapped to how patients actually describe their trouble. One query can land you a protocol, a formulation, or both.
              </p>
            </div>
          </Reveal>
          <div className="flex flex-col gap-6">
            <Reveal delay={200}>
              <button onClick={() => navigate({ name: "quiz" })} className="group relative w-full overflow-hidden rounded-xl border border-vata-500/40 bg-gradient-to-br from-vata-500/12 to-transparent p-6 text-left transition-all duration-500 hover:-translate-y-1 hover:border-vata-400">
            ...
            <Reveal delay={300}>
              <button onClick={() => navigate({ name: "herbs" })} className="group relative w-full overflow-hidden rounded-xl border border-kapha-500/40 bg-gradient-to-br from-kapha-500/12 to-transparent p-6 text-left transition-all duration-500 hover:-translate-y-1 hover:border-kapha-400">
                ...
                <span className="mt-2 block text-sm text-sand-200/60">Ashwagandha to Arjuna — energetics, doses and cautions, side by side.</span>
    ...
    <section className="relative overflow-hidden border-y border-forest-800 bg-forest-900">
      ...
      <div className="relative mx-auto max-w-3xl px-5 py-16 text-center lg:py-20">
        <Reveal>
          ...
          <form className="mx-auto mt-8 flex max-w-md gap-2" onSubmit={(e) => {
            e.preventDefault();
            ...
            <button type="submit" aria-label="Subscribe" className="grid h-[46px] w-[46px] shrink-0 place-items-center rounded-full bg-gold-400 text-forest-950 transition-transform hover:scale-105 active:scale-95">
              <Send size={18} />
            </button>
          </form>
